function load() {
    ...
    table = $('#tabla').DataTable({
        "ajax": {
            "url": "clientes/getListadoClientes"
        },
        "columns": [
            {"data": "id"},
            {"data": "razon_social"},
            ...
            {"data": "estatus",class:"centrado_text",
                render:function(data,type,row){
                    ...
                    if(data!=1)
                    {
                        var suspender_activar = '<a class="pmd-floating-action-btn btn btn-sm pmd-btn-fab pmd-btn-raised pmd-ripple-effect btn-default activar white classBotonFratsa" data-title="Activar" onclick="modalActivaCliente('+row.id+')"> <i class="ft-check"></i> </a>';
                    }  
                    else 
                    {
    ...
    $(document).ready(function () {
        ...
        $('#tabla tbody').on('click', 'a.edit', function () {
            ...
            var data = row.data();
            window.location = 'clientes/edicion_cliente/'+data.id;
        });
        ...
        $('#tabla tbody').on('click', 'a.visualiza', function () 
        {
            ...
            var row = table.row(tr);
            var data = row.data();
            window.location = 'clientes/visualiza_cliente/'+data.id;
        });
